...
-- | PackedOsrContent.
--
-- Pseudorange and carrier phase corrections for a satellite being tracked.
...
--
-- The OSR correction contains network corrections in an observation-like
-- format
data MsgOsr = MsgOsr
  { _msgOsr_header :: !ObservationHeader
    -- ^ Header of a GPS observation message
  , _msgOsr_obs  :: ![PackedOsrContent]
    -- ^ OSR correction for a satellite being tracked.
  } deriving ( Show, Read, Eq )
